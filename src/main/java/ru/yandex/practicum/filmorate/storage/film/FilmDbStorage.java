...
public class FilmDbStorage implements FilmStorage {
    ...
    public Film addFilm(Film film) {
        ...
        String sqlQuery = "INSERT INTO films (id, name, description, release_date, duration, MPA_id) " +
                "VALUES(?, ?, ?, ?, ?, ?)";
        //Сначала сохраняем данные в базу с основной информацией о фильме
        jdbcTemplate.update(sqlQuery,
                ...
                film.getDuration(),
                film.getMpa().getId());
        //Отдельно записываем информацию о жанрах фильма
        saveGenresOfFilmInDB(film);
        //Отдельно добавляем пустую запись в таблицу likes, чтоб фильм появлялся в топах при отсутствии лайков, но при пересчете лайков эта запись не учитывалась
    ...
    public Film updateFilm(Film film) {
        ...
        String sqlQuery = "UPDATE films SET name = ?, description = ?, release_date = ?, duration = ?, " +
                "MPA_id = ? WHERE id = ?";
        //Сначала перезаписываем данные в основную таблицу
        jdbcTemplate.update(sqlQuery,
                film.getName(),
                film.getDescription(),
                film.getReleaseDate(),
                ...
                film.getMpa().getId(),
                film.getId());
        //После этого удаляем данные жанров и сохраняем новые
    ...
    public void deleteFilmByID(Integer id) {
        //Данный функционал в ТЗ не требуется, но у меня уже есть реализации, поэтому оставляю тут пусто
    }
